...
const QuickAccessCard = () => {
  ...
  const quickAccessItems = [
    {
      ...
      onClick: () => navigate("/dashboard/apply-loan"),
    },
    {
      label: "Repayments",
  ...
  return (
    <>
          ...
          {quickAccessItems.map((item, index) => (
            <div key={index} className="flex flex-col items-center space-y-2">
              <button
                ...
                className="bg-white w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold hover:shadow-md transition-shadow"
              >
                <svg
      ...
      {/* Drawer */}
      <RepaymentDrawer open={openDrawer} onClose={() => setOpenDrawer(false)} />
    </>
  );
};
