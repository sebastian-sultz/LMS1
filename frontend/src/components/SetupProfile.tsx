...
const SetupProfile = (): JSX.Element => {
  ...
  // Calculate the latest selectable date (18 years ago from today)
  const latestSelectableDate = subYears(new Date(), 18);
                  ...
                  >
                    <div className="bg-white rounded-xl">
                      <Calendar
                        ...
                        selected={date}
                        onSelect={setDate}
                        defaultMonth={latestSelectableDate}
                        className="rounded-md border shadow-sm"
                        disabled={(date) =>
                          date > latestSelectableDate || date < new Date("1900-01-01")
                        }
